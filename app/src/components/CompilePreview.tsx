...
export function CompilePreview({ open, onOpenChange, changes, rawPaths, onApplied }: CompilePreviewProps) {
        ...
        <ScrollArea className="flex-1 mt-2">
          <div className="space-y-3">
            {changes.map((change, i) => (
              <div key={i} className={`border rounded-lg p-3 transition-opacity ${selected.has(i) ? "" : "opacity-40"}`}>
                <div className="flex items-center gap-2 mb-2">
                  <input
                    ...
                    className="rounded"
                  />
                  <span className="text-sm font-medium font-mono">{change.filename}</span>
                ...
                <pre className="text-xs bg-muted rounded-md p-2 max-h-40 overflow-auto font-mono whitespace-pre-wrap">
                  {change.content.slice(0, 500)}{change.content.length > 500 ? "..." : ""}
                </pre>
              </div>
            ))}
          </div>
